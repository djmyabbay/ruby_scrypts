...
print("no.,gene-code,gene,affiliation,root,root-affiliation,OTU_num,C_num,R_num,G_num,P_num\n")
queries = Array::new
	...
	begin
		...
		C_num = open(fasta_file).read.scan(/Cyanobacteria/).count
	       ...
	       G_num = open(fasta_file).read.scan(/Viridiplantae/).count
              P_num = open(fasta_file).read.scan(/Pavlova/).count
		print("\n#{i},#{gene_code},#{gene},#{affiliation},#{root},#{root_affiliation},#{OTU_num},#{C_num},#{R_num},#{G_num},#{P_num}")
	rescue Exception => e
		print("\n#{i},ERROR!!")
	end
end
