...
aligned3.alignment_collect do |otu| 
	...
	if gap_each_seq >= 0.3
		aligned3.remove_seq(otu) if (aligned3.index(otu) !~ /^QUERY/)&&(aligned3.index(otu) !~ /Lotharella/)&&(aligned3.index(otu) !~ /Cyanobacteria/)
	end
end
...
aligned5.alignment_collect do |otu2|
        ...
        if gap_each_seq2 >= 0.3
                aligned5.remove_seq(otu2) if (aligned5.index(otu2) !~ /^QUERY/)&&(aligned5.index(otu2) !~ /Lotharella/)&&(aligned3.index(otu) !~ /Cyanobacteria/)
        end
end
